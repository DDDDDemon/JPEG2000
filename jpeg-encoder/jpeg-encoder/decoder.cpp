...
extern unsigned char maxWidth, maxHeight;
extern int quantTable[4][128];
extern double cos_cache[200];
extern std::map<std::pair<unsigned char, unsigned int>, unsigned char> huffTable[2][2];
...
//            qDebug()<<huffCode[i].first<<" "<<huffCode[i].second<<": "<<v;
        }
        free(huffCode);
